...
public class FacturaCompraDetalleDAO {
    ...
    private static final String SQL_SELECT = "SELECT correlativo, PK_codigo_factura, PK_codigo_producto, PK_codigo_bodega, cantidad_detalle, costo_detalle, total_detalle FROM tbl_compra_detalle";
    private static final String SQL_INSERT = "INSERT INTO tbl_compra_detalle (correlativo, PK_codigo_factura, PK_codigo_producto, PK_codigo_bodega, cantidad_detalle, costo_detalle, total_detalle) VALUES(?,?,?,?,?,?,?)";
    private static final String SQL_UPDATE = "UPDATE tbl_compra_detalle SET  PK_codigo_factura=?, PK_codigo_producto=?, PK_codigo_bodega=?, cantidad_detalle=?, costo_detalle=?, total_detalle=?     WHERE correlativo=?";
    private static final String SQL_QUERY = "SELECT correlativo, PK_codigo_factura, PK_codigo_producto, PK_codigo_bodega, cantidad_detalle, costo_detalle, total_detalle FROM tbl_compra_detalle WHERE correlativo=?";
    private static final String SQL_DELETE = "DELETE FROM tbl_compra_detalle WHERE correlativo=?";

    public List<FacturaCompraDetalle> select() {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        FacturaCompraDetalle proveedor = null;
        List<FacturaCompraDetalle> proveedores = new ArrayList<FacturaCompraDetalle>();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                proveedor = new FacturaCompraDetalle();
    ...
    public int insert(FacturaCompraDetalle proveedor) {
        Connection conn = null;
        PreparedStatement stmt = null;
        int rows = 0;
        try {
            ...
            stmt.setInt(2, proveedor.getPK_Codigo_Factura());
    ...
    public FacturaCompraDetalle query(FacturaCompraDetalle proveedor) {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        int rows = 0;
...
//            System.out.println("Ejecutando query:" + SQL_QUERY);
    ...
    public int delete(FacturaCompraDetalle aplicacion) {
    ...
    public int update(FacturaCompraDetalle proveedor) {
        Connection conn = null;
        PreparedStatement stmt = null;
        int rows = 0;
